...
async function refreshAccessToken() {
  ...
  return new Promise((resolve, reject) => {
    axios
      ...
      .then((response) => {
        const now = new Date();
        const expiration = now.setSeconds(now.getSeconds() + response.data.expires_in);
...
router.get("/", (req, res) => {
  ...
  function performPostRequest() {
    ...
    return new Promise((resolve, reject) => {
      axios
        .post("https://api.fitbit.com/oauth2/token", postData, { headers })
        .then((response) => {
          const expiration = calculateExpirationTimeOfAccessToken(
  ...
  if (!tokenStore.getAccessToken() || tokenStore.isExpired()) {
    performPostRequest()
      .then((accessToken) => {
